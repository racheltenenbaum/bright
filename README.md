...
**Sun-and-shade-aware walking directions.** bright plans routes and finds cafés, parks, bars, and restaurants optimized for whether you want to walk in the sun or stay in the shade — right now, or at a specific time of day.

Instead of the shortest path, bright computes real-time solar geometry, casts building shadows using OpenStreetMap footprints, and re-weights every street segment in the route graph so the shortest *sunny* (or shadiest) path wins — not just the shortest path.
...
- **Sun/shade-optimized routing** — plan a walking route between two addresses that favors sunlit or shaded streets, with a configurable max-detour tolerance (%) traded against pure distance.
...
- **Find Places** — search nearby cafés, restaurants, bars, and parks filtered by sunny/shaded preference, with an evening auto-fallback (café → "bar" reclassification) and review/quality filtering.
- **Live weather + sun position** for any point, pulled from Google's Weather API and merged with locally computed solar altitude.
- **Saved Routes & Saved Spots** — authenticated users can save routes and favorite places, each independently shareable via a public UUID share link (no login required to view a shared route/spot).
- **Per-user preferences** — sun vs. shade default, max detour tolerance, map type (roadmap/satellite/terrain/hybrid), and whether map UI controls are shown — all persisted server-side.
...
- API keys: Google Maps (Places, Weather, Elevation, JS API) — Overpass requires no key
...
*bright — because the best walk is the one where you're not squinting the whole time.*
